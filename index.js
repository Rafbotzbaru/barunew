...
const connectToWhatsApp = async () => {
	...
	let metadata = await conn.groupMetadata(data.id)
	  for (let i of data.participants) {
		...
		if (data.action == "add") {
		  conn.sendMessage(data.id, { image: { url: pp_user }, caption: `Hy @${i.split("@")[0]} Yatim\nWelcome To Group Bokep\nMasuk Gc Gw Ajg :\nG Masuk? Piatu\n\n\₲ⱤɄ₱ ฿Ø₮Ⱬ ꋬꇙꌦꋪꋬꊰ 6ꌦꄲ꒤꒤¹😈\nGROUP 1:\nhttps://chat.whatsapp.com/CRrj0z3HgeX39XHNUt0M9V\n\nGROUP 2:\nhttps://chat.whatsapp.com/Da7ZPf7mC8T3nERBpWPcgt\n\nGROUP 3:\nhttps://chat.whatsapp.com/BdwNQqYBwDN5UtvievnDEe`, mentions: [i] })
		} else if (data.action == "remove") {
		  conn.sendMessage(data.id, { image: { url: pp_user }, caption: `Goodbye @${i.split("@")[0]} Yatim\nJangan Pernah Semangat\nTetap Putus Asa\nDan Jadilah Yatim Beban Keluarga\n💪🗿`, mentions: [i] })
		}
	  }
	} catch (e) {
	  console.log(e)
